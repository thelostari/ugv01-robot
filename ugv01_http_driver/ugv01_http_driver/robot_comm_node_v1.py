...
class UGV01HttpDriver(Node):
    def __init__(self) -> None:
        ...
        self.cmd_vel_timer = self.create_timer(
            timer_period_sec=(1.0 / self.cmd_vel_rate), callback=self.cmd_vel_timer_callback
        )
        self.imu_timer = self.create_timer(
            timer_period_sec=(1.0 / self.imu_rate), callback=self.imu_timer_callback
        )
    ...
    def cmd_vel_timer_callback(self) -> None:
        """
        Called periodically at cmd_vel_rate Hz to add a velocity command to the queue.
        """
    ...
    def imu_timer_callback(self) -> None:
        """
        Called periodically at imu_rate Hz.
        """
        ...
        self.enqueue_job(priority=1, category="read_imu")
